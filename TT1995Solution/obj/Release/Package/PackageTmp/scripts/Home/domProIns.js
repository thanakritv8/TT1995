...
var columnHide = [];
var gbTableId = '16';
var tableName = "domestic_product_insurance";
var idFile;
...
$(function () {
    ...
    dataGridAll = getDataDpi();

    //data grid
    var dataGrid = $("#gridContainer").dxDataGrid({

        dataSource: getDataDpi(),
        searchPanel: {
            visible: true,
            width: 240,
            placeholder: "Search..."
        ...
        editing: {
            ...
            popup: {
                title: "รายการประกันสินค้าภายในประเทศ",
                showTitle: true,
                width: "70%",
        ...
        masterDetail: {
            ...
            template: function (container, options) {
                ...
                var itemData = fnGetFiles(options.key.dpi_id, gbTableId);
                ...
                treeview = $("#treeview").dxTreeView({
                    ...
                    onItemClick: function (e) {
                        gallery = [];
                        itemData = fnGetFiles(options.key.dpi_id, gbTableId);
                        var item = e.itemData;
                        //console.log(e);
                        if (item.path_file) {
                ...
                //จบการสร้าง treeview
                fnChangeTreeview(options.key.dpi_id, itemData);
            }
        },
        ...
        onRowClick: function (e) {
            if (gbE.currentSelectedRowKeys[0].dpi_id == e.key.dpi_id && isFirstClick && rowIndex == e.rowIndex && gbE.currentDeselectedRowKeys.length == 0) {
                dataGrid.clearSelection();
            } else if (gbE.currentSelectedRowKeys[0].dpi_id == e.key.dpi_id && !isFirstClick) {
                isFirstClick = true;
                rowIndex = e.rowIndex;
            }
        },
    ...
    function fnGetFiles(DPIId, IdTable) {
        //alert(PICId + IdTable);
        var itemData;
        $.ajax({
            type: "POST",
    ...
    function fnChangeTreeview(dpi_id, itemData) {
        var nItem = 0;
                        ...
                        $('<a style="color:green;font-weight:bold;" />').addClass('dx-link')
                                ...
                                .on('dxclick', function (e) {
                                    ...
                                    var gridHistory = $("#gridHistory").dxDataGrid({
                                        dataSource: fnGetHistory(gbTableId, options.row.data.dpi_id),
                                        showBorders: true,
                                        height: 'auto',
                                        scrolling: {
                                            mode: "virtual"
    ...
    function fnUpdateDPI(newData, keyItem) {
        //console.log(keyItem);
        newData.key = keyItem;
    ...
    $("#btnNewFolder").dxButton({
        ...
        onClick: function () {
            ...
            if (folderName != "") {
                fileDataDpi = new FormData();
                fileDataDpi.append('fk_id', idFK);
                fileDataDpi.append('parentDirId', idFile);
                fileDataDpi.append('newFolder', folderName);
                fileDataDpi.append('tableId', gbTableId);
                fileDataDpi.append('tableName', tableName);
                fnInsertFiles(fileDataDpi);
            } else {
    ...
    $("#btnRename").click(function () {
        ...
        if (folderName != "") {
            fileDataDpi = new FormData();
            fileDataDpi.append('fk_id', idFK);
            fileDataDpi.append('file_id', idFile);
            fileDataDpi.append('rename', folderName);
            fnRename(fileDataDpi);
        } else {
            DevExpress.ui.notify("กรุณากรอกชื่อโฟล์เดอร์", "error");
        }
    });
                        ...
                        reader.onload = function (e) {
                            ...
                            img.onload = function () {
                                ...
                                fetch(dataurl)
                                    .then(res => res.blob())
                                    .then(blob => {
                                        fileDataDpi.append('file', blob, file.name);
                                    });
                            }
                        }
