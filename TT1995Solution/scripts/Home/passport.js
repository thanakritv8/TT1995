...
$(function () {
    $("a:contains('พาสสปอร์ต')").first().addClass("active");
    ...
    var cf = $(".custom-file").dxFileUploader({
        ...
        onValueChanged: function (e) {
            ...
            if (files.length > 0) {
                ...
                fileDataPdf.append('fk_id', gbE.currentSelectedRowKeys[0].pas_id);
            }
        },
    ...
    var dataGrid = $("#gridContainer").dxDataGrid({
        ...
        onRowUpdating: function (e) {
            if (!fnUpdatePAS(e.newData, e.key.pas_id)) {
                e.cancel = true;
            }
            setDefaultLookupDriver();
        },
        onRowInserting: function (e) {
            var idInsert = fnInsertPAS(e.data);
            if (idInsert != 0) {
                e.data.pas_id = idInsert;
        ...
        onRowRemoving: function (e) {
            e.cancel = fnDeletePAS(e.key.pas_id);
        },
        masterDetail: {
            enabled: false,
                        ...
                        $('<a style="color:green;font-weight:bold;" />').addClass('dx-link')
                            ...
                            .on('dxclick', function (e) {
                                ...
                                var gridHistory = $("#gridHistory").dxDataGrid({
                                    dataSource: fnGetHistory(gbTableId, options.row.data.pas_id),
                                    showBorders: true,
                                    height: 'auto',
                                    scrolling: {
                                        mode: "virtual"
    ...
    function fnUpdatePAS(newData, keyItem) {
        //console.log(keyItem);
        newData.key = keyItem;
