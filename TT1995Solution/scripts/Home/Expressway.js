...
$(function () {
    ...
    dataGridAll = GetExpresswayData();

    //data grid
    var dataGrid = $("#gridContainer").dxDataGrid({
        dataSource: GetExpresswayData(),
        ...
        masterDetail: {
            ...
            template: function (container, options) {
                ...
                var itemData = fnGetFiles(options.key.epw_id, gbTableId);
                ...
                treeview = $("#treeview").dxTreeView({
                    ...
                    onItemClick: function (e) {
                        gallery = [];
                        itemData = fnGetFiles(options.key.epw_id, gbTableId);
                        var item = e.itemData;
                        console.log(e);
                        if (item.path_file) {
                ...
                //จบการสร้าง treeview
                fnChangeTreeview(options.key.epw_id, itemData);
            }
        },
        ...
        onRowClick: function (e) {
            if (gbE.currentSelectedRowKeys[0].epw_id == e.key.epw_id && isFirstClick && rowIndex == e.rowIndex && gbE.currentDeselectedRowKeys.length == 0) {
                dataGrid.clearSelection();
            } else if (gbE.currentSelectedRowKeys[0].epw_id == e.key.epw_id && !isFirstClick) {
                isFirstClick = true;
                rowIndex = e.rowIndex;
            }
        },
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.forEach(function (item) {
                        ...
                        $('<a style="color:green;font-weight:bold;" />').addClass('dx-link')
                                ...
                                .on('dxclick', function (e) {
                                    ...
                                    var gridHistory = $("#gridHistory").dxDataGrid({
                                        dataSource: fnGetHistory(gbTableId, options.row.data.epw_id),
                                        showBorders: true,
                                        height: 'auto',
                                        scrolling: {
                                            mode: "virtual"
                ...
                if (item.dataField != "create_date" && item.dataField != "create_by_user_id" && item.dataField != "update_date" && item.dataField != "update_by_user_id" && item.dataField != "epw_id" && item.dataField != "history") {
    ...
    function fnGetFiles(ExpresswayId, IdTable) {
        var itemData;
        $.ajax({
            type: "POST",
            ...
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: "{Id: " + ExpresswayId + ",IdTable: " + IdTable + "}",
            async: false,
            success: function (data) {
                data.push({
                    "file_id": "root",
    ...
    function fnChangeTreeview(epw_id, itemData) {
        var nItem = 0;
    ...
    function fnDeleteFiles(file_id) {
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: "{keyId: '" + file_id + "',FolderName:'Expressway'}",
            dataType: 'json',
            success: function (data) {
                if (data[0].Status != '0') {
    ...
    function fnInsertExpressway(dataGrid) {
        dataGrid.IdTable = gbTableId;
        var returnId = 0;
        $.ajax({
            type: "POST",
            url: "../Home/InsertExpressway",
            contentType: "application/json; charset=utf-8",
